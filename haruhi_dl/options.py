...
def parseOpts(overrideArguments=None):
    ...
    def _readUserConf():
        ...
        if xdg_config_home:
            userConfFile = os.path.join(xdg_config_home, 'haruhi-dl', 'config')
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(xdg_config_home, 'haruhi-dl.conf')
        else:
            userConfFile = os.path.join(compat_expanduser('~'), '.config', 'haruhi-dl', 'config')
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(compat_expanduser('~'), '.config', 'haruhi-dl.conf')
        ...
        if userConf is None:
            ...
            if appdata_dir:
                userConf = _readOptions(
                    os.path.join(appdata_dir, 'haruhi-dl', 'config'),
                    default=None)
                if userConf is None:
                    userConf = _readOptions(
                        os.path.join(appdata_dir, 'haruhi-dl', 'config.txt'),
                        default=None)

        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser('~'), 'haruhi-dl.conf'),
                default=None)
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser('~'), 'haruhi-dl.conf.txt'),
                default=None)
    ...
    general.add_option(
        ...
        'When given in the global configuration file /etc/haruhi-dl.conf: '
        'Do not read the user configuration in ~/.config/haruhi-dl/config '
        '(%APPDATA%/haruhi-dl/config.txt on Windows)')
    general.add_option(
        '--config-location',
    ...
    authentication.add_option(
        ...
        help='Account password. If this option is left out, haruhi-dl will ask interactively.')
    authentication.add_option(
        '-2', '--twofactor',
    ...
    adobe_pass.add_option(
        ...
        help='Multiple-system operator account password. If this option is left out, haruhi-dl will ask interactively.')
    adobe_pass.add_option(
        '--ap-list-mso',
    ...
    downloader.add_option(
        ...
        help='Set file xattribute hdl.filesize with expected file size')
    downloader.add_option(
        '--hls-prefer-native',
    ...
    verbosity.add_option(
        ...
        help='Contact the haruhi-dl server for debugging')
    verbosity.add_option(
        '--no-call-home',
    ...
    filesystem.add_option(
        ...
        help='Force resume of partially downloaded files. By default, haruhi-dl will resume downloads if possible.')
    filesystem.add_option(
        '--no-continue',
    ...
    filesystem.add_option(
        ...
        help='Location in the filesystem where haruhi-dl can store some downloaded information permanently. By default $XDG_CACHE_HOME/haruhi-dl or ~/.cache/haruhi-dl . At the moment, only YouTube player files (for videos with obfuscated signatures) are cached, but that may change.')
    filesystem.add_option(
    ...
    if overrideArguments is not None:
        ...
        if '--config-location' in command_line_conf:
            ...
            if os.path.isdir(location):
                location = os.path.join(location, 'haruhi-dl.conf')
            ...
            system_conf = _readOptions('/etc/haruhi-dl.conf')
