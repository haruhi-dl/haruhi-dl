...
def update_self(to_screen, verbose, opener):
    ...
    if not isinstance(globals().get('__loader__'), zipimporter) and not hasattr(sys, 'frozen'):
        to_screen('It looks like you installed haruhi-dl with a package manager, pip, setup.py or a tarball. Please use that to update.')
        return
    ...
    if newversion == __version__:
        to_screen('haruhi-dl is up-to-date (' + __version__ + ')')
        return
    ...
    if version_tuple(__version__) >= version_tuple(version_id):
        to_screen('haruhi-dl is up to date (%s)' % __version__)
        return
    ...
    if hasattr(sys, 'frozen'):
        ...
        try:
            bat = os.path.join(directory, 'haruhi-dl-updater.bat')
...
move /Y "%s.new" "%s" > NUL
echo Updated haruhi-dl to version %s.
start /b "" cmd /c del "%%~f0"&exit /b"
    ...
    to_screen('Updated haruhi-dl. Restart haruhi-dl to use the new version.')
